<?php
/**
 * Team post type endpoint and functions.
 *
 * @package    WordPress
 * @subpackage Dracobit
 * @since      1.0.0
...
class WP_JSON_Team {
   ...
   public function register_routes( $routes ) {
     $routes['/team'] = array(
       array( array( $this, 'get_teams' ), WP_JSON_Server::READABLE ),
       array( array( $this, 'create_team' ), WP_JSON_Server::CREATABLE | WP_JSON_Server::ACCEPT_JSON ),
     );

     $routes['/team/(?P<id>\d+)'] = array(
       array( array( $this, 'get_team' ), WP_JSON_Server::READABLE ),
       array( array( $this, 'edit_team' ), WP_JSON_Server::EDITABLE | WP_JSON_Server::ACCEPT_JSON ),
     );
   ...
   public function update_post_meta( $post, $data, $update ) {
    ...
    //  }
     ...
     if ( isset( $data['team_tier'] ) ) {
       update_post_meta( $post['ID'], 'team_tier', $data['team_tier'] );
     }
  ...
  /**
	 ...
	 * @return WP_JSON_Response $wp_json_posts All teams
	 */
	public function get_teams( $filter = array(), $page = 1 ) {
		global $wp_json_posts;
		return $wp_json_posts->get_posts( $filter, 'view', 'team', $page );
	}
  ...
  /**
	 ...
	 * @return WP_JSON_Response $wp_json_posts Specific Team
	 */
  ...
  /**
	 ...
	 * @since  1.0.0
	 * @param  array                     $data          Data from API
	 * @return WP_JSON_Response/WP_Error $wp_json_posts Updated posts table with new team/Login error
	 */
  ...
  /**
	 ...
	 * @return WP_JSON_Response $wp_json_posts Updated posts table with edited team
	 */
   public function edit_team( $id, $data, $_header ) {
     global $wp_json_posts;
